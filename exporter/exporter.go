...
func NewExporter(uri, name string) (*Exporter, error) {
	addr := uri
	password := os.Getenv("PREDIXY_PASSWORD")

	labels := prometheus.Labels{"cluster": name, "addr": addr}
	latencyDesc := prometheus.NewDesc(namespace+"_latency", "Latency", latencyLabels, labels)
	servLatencyDesc := prometheus.NewDesc(namespace+"_server_latency", "Server latency", servLatencyLabels, labels)
	...
	e := &Exporter{
		...
		clusterGauges:   map[string]prometheus.Gauge{},
		servGauges:      map[string]*prometheus.GaugeVec{},
		latencyDesc:     latencyDesc,
		servLatencyDesc: servLatencyDesc,
		latencyMetrics:  []prometheus.Metric{},
	}
...
// scrape connect predixy and fetch INFO
